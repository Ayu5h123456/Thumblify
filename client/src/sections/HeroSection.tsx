...
export default function HeroSection() {
    ...
    return (
        <div className="relative flex flex-col items-center justify-center px-4 md:px-16 lg:px-24 xl:px-32">
            <div className="absolute top-30 -z-10 left-1/4 size-72 bg-gray-600 blur-[300px]"></div>
            <motion.a href="https://prebuiltui.com?utm_source=pixels" className="group flex items-center gap-2 rounded-full p-1 pr-3 mt-44 text-gray-100 bg-gray-200/15"
                initial={{ y: -20, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                viewport={{ once: true }}
                transition={{ delay: 0.2, type: "spring", stiffness: 320, damping: 70, mass: 1 }}
            >
                <span className="bg-gray-800 text-white text-xs px-3.5 py-1 rounded-full">
                    NEW
                </span>
            ...
            >
                <button onClick={()=>navigate('/generate')} className="bg-gray-600 hover:bg-gray-700 text-white rounded-full px-7 h-11">
                    Generate now
                </button>
                <button className="flex items-center gap-2 border border-gray-900 hover:bg-gray-950/50 transition rounded-full px-6 h-11">
            ...
            <div className="flex flex-wrap justify-center items-center gap-4 md:gap-14 mt-12">
                {specialFeatures.map((feature, index) => (
                    ...
                    >
                        <CheckIcon className="size-5 text-gray-600" />
